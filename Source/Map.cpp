...
bool Map::LoadFromFile(const std::string& filename)//двоеточия-обращение к методам класса вне класса
{
...
Object Map::GetObject(const std::string& name)
{
...
std::vector<Object> Map::GetObjects(const std::string& name)
{
...
void Map::Draw(sf::RenderTexture& window)
{
  ...
  for (std::size_t layer = 0; layer < layers.size(); layer++)
    for (std::size_t tile = 0; tile < layers[layer].tiles.size(); tile++)
